	...
	bindEditFunctionality() {
		...
		$(document).on('click', this.editButtonsSelector, (e) => {
			...
			for (let i in options) {
				$editModalOptions
					.find(`[data-option-title='${options[i].name}'] + div label[data-swatch-value='${options[i].value}'] input`)
						.trigger('click')
						.trigger('mouseover')
						.trigger('mouseout');
			...
			oldFormArray = self.$editModal.find('form').serialize().split("&");
		...
		this.$editModal.on('submit', 'form', (e) => {
			...
			const newFormArray = $(e.target).serialize().split("&");
			const hasFormArrayChanged = oldFormArray.filter((item) => { return newFormArray.indexOf(item) !== -1 });

			// console.log(hasFormArrayChanged.length !== oldFormArray.length)

			if(hasFormArrayChanged.length !== oldFormArray.length){
